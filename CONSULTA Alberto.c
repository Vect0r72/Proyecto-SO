...
int main(int argc, char **argv)
{
	...
	// de la base de datos
	err=mysql_query (conn, "SELECT * FROM db");
	if (err!=0) {
	...
	if (row == NULL)
		...
		while (row !=NULL) {
			...
			printf ("Partidas: %s, nombre: %s, edad: %d\n", row[0], row[1], edad);
		...
		// dado por el usuario
		printf ("Quieres saber cuantas partidas jugo Alberto?"); 
		scanf ("%s", respuesta);
		// construimos la consulta SQL
		strcpy (consulta,"SELECT Historial.idp From Jugadores,Historial WHERE Jugadores.Username = 'Alberto' AND Historial.idj = Jugadores.ID"); 
		strcat (consulta, dni);
		strcat (consulta,"'");
		// hacemos la consulta 
		...
		if (row == NULL)
			...
			printf ("Alberto jugó %s partidas\n", row[0] );
